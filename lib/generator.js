...
function parseStatement(statement, declarations) {
  ...
  if (isClass || isInterface) {
    classInfo = {
      ...
      interfaces: getImplementedInterfaces(doc.tags),
...
function getImplementedInterfaces(tags) {
  return tags.filter(function(tag) {
    return tag.title === 'implements';
  }).map(function(tag) {
    return getTsType(tag.type);
  });
}
...
function outputClassDeclaration(indent, declare) {
  ...
  if (declare.type === 'ClassType') {
    var impl = '';
    if (declare.interfaces.length > 0) {
      impl = ' implements ' + declare.interfaces.join(', ');
    }
    output.push('export class ' + declare.name + getTemplateString(declare.templates) + extend + impl + ' {');
